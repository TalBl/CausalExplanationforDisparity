...
def create_colored_dot(value):
    color = f"rgb({255 - int(value * 255)}, {int(value * 255)}, 0)"  # Red-to-Green transition
...
file = output_col = group1_query = group2_query = file_dag = imutable_atts = mutable_atts = clicked = None
    ...
    with col1:
        ...
        if file is None:
            ...
            if mutable_atts:
                ...
                filter_scenario = st.selectbox("Choose filter scenario:", ['Investigate a disparate trend', 'Debugging bias', 'Discovering reverse trends'], key='filter_scenario')
                clicked = st.button("Explain", key="Explain")
        if file and output_col and group1_query and group2_query and imutable_atts and mutable_atts and clicked:
                calc_algorithm()
    with col2:
        ...
        if valid_group('Group_A'):
            ...
            with group_avg_col:
                ...
                st.write(f"**Average Group A:** {group_a_avg:.2f}  **Average Group B:** {group_b_avg:.2f}")
            ...
            df2["ATE1"] = df2["ate1"].apply(format_currency)
            df2["ATE2"] = df2["ate2"].apply(format_currency)
            df2 = df2[["subpopulation", "treatment", "support", "ATE1", "ate_group1", "ATE2", "ate_group2"]]
