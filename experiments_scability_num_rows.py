...
acs = Dataset(name="acs", outcome_col="Health insurance coverage recode",
              ...
              columns_to_ignore=[], clean_path="outputs/acs/sample_data2.csv", func_filter_subs=acs_filter_subs, need_filter_subpopulations=True, need_filter_treatments=True,
...
def run_test(d: Dataset, count_rows):
    ...
    df_sample.to_csv(f"outputs/{dd.name}/sample_data_{str_count}.csv", index=False)
    dd.clean_path = f"outputs/{dd.name}/sample_data_{str_count}.csv"
    start_time = time.time()
    algorithm(D=dd)
    end_time = time.time()
    runtime = end_time-start_time
    return {"dataset": d.name,"COUNT ROWS": frac, "runtime": round(runtime, 2)}
...
l = []
for count_rows in [(0.25, "25"), (0.5, "50"), (0.75, "75"), (1, "100")]:
...
save_graph(pd.DataFrame(l), "COUNT ROWS")
