...
class Dataset_RF():
    def __init__(self, root = None, rootTrain = None, rootTest = None, column_names = None):
        ...
        if self.root == None and (self.rootTrain == None or self.rootTest == None):
            raise ValueError("Dataset_RF root path not provided")
...
class FairnessDebuggingUsingMachineUnlearning():
    ...
    '''
    def __init__(self, dataloader, sensitiveAttribute, classLabel, fairnessMetric, DAG, group1_idx, group2_idx):
        self.dataloader = dataloader
        if self.dataloader == None:
            raise ValueError("Dataset root path not provided")
        if not isinstance(self.dataloader, Dataset_RF):
        ...
        self.DAG = DAG
        self.group1_idx = group1_idx
        self.group2_idx = group2_idx
        self.__loadDataset()
    ...
    def getDatasetFairnessParity(self):
        ...
        elif self.fairnessMetric == FairnessMetric.CATE:
            return self.getDatasetCATE()
    ...
    def __loadDataset(self):
        ...
        self.group1TestIndices = self.group1_idx
        self.group2TestIndices = self.group2_idx
        self.categorizedTrain, self.categorizedTest = self.dataloader.getDatasetWithCategorizationPreprocessing(decodeAttributeValues = False)
        self.columns = self.trainX.columns.values
        '''Feature scaling to standardize dataset to help model learn patterns'''
        for col in self.trainX.columns:
            if col == "TempTreatment":
                continue
    ...
    def __getPredictions(self):
        ...
        self.CATE = self.__getCATEDifference(self.predictions)
    ...
    def __getFairnessParityDifference(self, priviledgedIndices, protectedIndices, predictions):
        if self.fairnessMetric == FairnessMetric.SP:
            ...
            return self.__getCATEDifference(predictions)
    ...
    def __getDiffMeans(self, priviledgedIndices, protectedIndices, predictions):
        return abs(np.mean(predictions[self.group1_idx]) - np.mean(predictions[self.group2_idx]))

    def getTreatmentATE(self, df_group):
        try:
            model = CausalModel(
                data=df_group,
                graph=self.DAG,
                treatment='TempTreatment',
                outcome=self.classLabel)
    ...
    def __getCATEDifference(self, predictions):
        df_test = self.testX.copy()
        df_group1 = df_test.iloc[self.group1TestIndices]
        df_group1[self.classLabel] = predictions[self.group1TestIndices]
        ate_group1 = self.getTreatmentATE(df_group1)
        df_group2 = df_test.iloc[self.group2TestIndices]
        df_group2[self.classLabel] = predictions[self.group2TestIndices]
        ate_group2 = self.getTreatmentATE(df_group2)
        cate_curr = abs(ate_group1 - ate_group2)

        df_group1[self.classLabel] = self.testY[self.group1TestIndices]
        ate_group1 = self.getTreatmentATE(df_group1)
        df_group2[self.classLabel] = self.testY[self.group2TestIndices]
        ate_group2 = self.getTreatmentATE(df_group2)
        cate_ori = abs(ate_group1 - ate_group2)
        return cate_curr - cate_ori
    ...
    def __createAttributeMap(self):
        ...
        self.literals = np.concatenate([categorizedDf[col].unique() for col in categorizedDf if col not in [self.classLabel, "TempTreatment"]])
