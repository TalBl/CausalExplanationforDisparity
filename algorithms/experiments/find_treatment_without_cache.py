...
def ComputeCATEnFilter(treats: set, d: Dataset, dag, depth, dict_res: dict, df_group1, df_group2, p_value):
    ...
    try:
        if depth - 1 >= 1:
            ...
            treatment_att = list(treats)[0][0]
            if treatment_att.startswith('DevType'):
                treatment_att = 'DevType'
            lines = changeDAG(dag, [treatment_att])
            dag = calc_dag(lines)
...
def run_subpopulations(d):
    ...
    with ThreadPoolExecutor() as executor:
        future_to_item = {executor.submit(findBestTreatment, s, d.copy(), dag, 0.25): s for s in subpopulations}
        for future in as_completed(future_to_item):
            i = future_to_item[future]
            try:
