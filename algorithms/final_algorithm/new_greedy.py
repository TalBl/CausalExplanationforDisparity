...
def calc_facts_metrics(dataset: Dataset, lamda=None):
    ...
    if not lamda:
        lamda = choose_lamda([x["iscore"] for x in results])
    df = pd.DataFrame(results)
    df['ni_score'] = df['iscore'].apply(lambda x: ni_score(x, lamda))
    return df


def get_intersection(att1, att2, d):
...
def get_score(max_subpopulation, group, attribute, alpha, d):
    intersection = 0
    checked_group = group.copy()
    checked_group.append(attribute)
    ni_score_sum = sum([i['ni_score'] for i in checked_group])
    utility = ni_score_sum / len(checked_group)
    if group:
        for pair in itertools.combinations(checked_group, 2):
            intersection += get_intersection(pair[0], pair[1], d)
...
def greedy(d, df_facts, max_subpopulation, alpha, K):
    j = 0
    group = []
    scores = []
    items = []
    ...
    while j < K:
        ...
        for _, group_rows in tqdm(df_facts.iterrows(), total=df_facts.shape[0]):
            if group_rows['subpopulation'] in items:
                continue
            combinations.append(group_rows)
        try:
            with ThreadPoolExecutor(max_workers=10) as executor2:
                future_to_item = {executor2.submit(get_score, max_subpopulation, group, i, alpha, d.copy()): i for i in combinations}
